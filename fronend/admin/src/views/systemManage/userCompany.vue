...
export default {
  ...
  methods: {
    ...
    getList() {
      userCompanyIdApi.list(this.listQuery).then(res => {
    ...
    createConfig() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          userCompanyIdApi.create(this.temp).then(res => {
    ...
    editConfig() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          userCompanyIdApi.update(this.temp).then(res => {
    ...
    del(row) {
      this.$confirm('是否删除该配置?', '提示', {
        ...
        userCompanyIdApi.delete(row.id).then(res => {
          this.getList()
        })
      }).catch(() => {})
    }
